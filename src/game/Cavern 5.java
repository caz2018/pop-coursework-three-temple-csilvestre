...
public class Cavern {
    ...
    public static Cavern digExploreCavern(int rows, int cols, Random rand) {
        var minOrbDist = minOrbDistance(rows, cols);

        var cavern = new Cavern(rows, cols, rand, () -> 1, () -> 0, Tile.Type.ORB);
    ...
    public static Cavern digEscapeCavern(int rows, int cols, int currentRow, int currentCol, Random rand) {
        ...
        var potentialCavern = new Cavern(rows, cols, rand, edgeWeightGen, goldGen, Tile.Type.ENTRANCE);
    ...
    private static int generateGoldValue(Random rand) {
        ...
        var val = rand.nextInt(MAX_GOLD_VALUE) + 1;
    ...
    public static Cavern deserialize(List<String> nodeStrList) {
        ...
        for (var nodeStr : nodeStrList) {
            ...
            var nodeInfo = nodeStr.substring(0, nodeStr.indexOf("="));
            var splitInfo = nodeInfo.split(",");

            var nodeId = Long.parseLong(splitInfo[0]);
        ...
        for (var nodeStr : nodeStrList) {
            ...
            var nodeId = Long.parseLong(nodeAndEdgeInfo[0].split(",")[0]);

            var n = idToNode.get(nodeId);
            tiles[n.getTile().getRow()][n.getTile().getColumn()] = n;
            for (var edgeStr : nodeAndEdgeInfo[1].split(",")) {
                String[] idAndWeight = edgeStr.split("-");
                var otherId = Long.parseLong(idAndWeight[0]);
                var weight = Integer.parseInt(idAndWeight[1]);
    ...
    private Set<Node> generateGraph(Random rand,
        ...
        var entrancePoint = getEntrancePoint(rand);
    ...
    public List<String> serialize() {
        ...
        for (var n : graph) {
            var t = n.getTile();
            var nodeStr = n.getId() + "," + t.getRow() + "," + t.getColumn() + "," + t.getGold() + "," + t.getType().name();

            var edges = n.getExits().stream()
